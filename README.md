...
* **Self-hosted stock APKs (archive.org)** — Facebook, Twitter, and Instagram stock APKs are mirrored on a self-hosted archive.org item because apkmirror 403s (and uptodown doesn't reliably serve) their builds. All three track `auto`: Facebook resolves to the only De-Vanced–supported build (currently `490.0.0.63.82`), served from the archive while available, falling back to uptodown if a newer resolved version isn't mirrored yet.
